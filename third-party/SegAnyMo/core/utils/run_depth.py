...
def get_depth_anything_disp_batch(
    pipe: Pipeline,
    img_files: List[str],
    ret_type: Literal["uint16", "float"] = "float",
):
    ...
    disp_outputs = pipe(images, batch_size=batch_size)
    torch.cuda.synchronize() # Ensure GPU work is done before stopping timer

    results = []

    for i, image in enumerate(images):
        disp = disp_outputs[i]["predicted_depth"] # This is likely already on GPU
        disp = torch.nn.functional.interpolate(
            disp.unsqueeze(1), size=image.size[::-1], mode="bicubic", align_corners=False
        )
        torch.cuda.synchronize() # Ensure interpolate is done
        disp_np = disp.squeeze().cpu().numpy()

        if ret_type == "uint16":
            results.append(to_uint16(disp_np))
        elif ret_type == "float":
            results.append(disp_np)
        else:
            raise ValueError(f"Unknown return type {ret_type}")
...
def save_disp_from_dir(
    ...
    for i in tqdm(range(0, len(img_files), batch_size), f"computing {model_name} depth maps"):
        ...
        for img_file in batch_files:
            ...
            if not osp.exists(out_file):
                batch_files_to_process.append(img_file)
        
        if not batch_files_to_process:
            continue
            
        disps = get_depth_anything_disp_batch(pipe, batch_files_to_process, ret_type="uint16")
        
        for j, img_file in enumerate(batch_files_to_process):
            out_file = osp.join(out_dir, osp.splitext(osp.basename(img_file))[0] + ".png")
            iio.imwrite(out_file, disps[j])
...
def main():
    ...
    save_disp_from_dir(
        args.model, args.img_dir, args.out_raw_dir, args.matching_pattern, args.step, args.batch_size
    )
