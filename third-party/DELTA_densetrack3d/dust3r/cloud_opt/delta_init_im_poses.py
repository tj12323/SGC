...
def init_minimum_spanning_tree(self, **kw):
    ...
    pts3d, _, im_focals, im_poses = minimum_spanning_tree(self.imshapes, self.edges,
                                                          self.pred_i, self.pred_j, self.conf_i, self.conf_j, self.dymask_i, self.dymask_j, self.im_conf, self.min_conf_thr,
...
def minimum_spanning_tree(imshapes, edges, pred_i, pred_j, conf_i, conf_j, dymask_i, dymask_j, im_conf, min_conf_thr,
    ...
    static_mask_i = ~(dymask_j[j_i].bool())
    # s, R, T = rigid_points_registration(pred_i[i_j], pts3d[i], conf=conf_i[i_j])
    s, R, T = rigid_points_registration(pred_j[j_i][static_mask_i], pts3d[i][static_mask_i], conf=conf_j[j_i][static_mask_i])

    # breakpoint()
    trf = sRT_to_4x4(s, R, T, device)
    # pts3d[j] = geotrf(trf, pred_j[i_j])
    pts3d[j] = geotrf(trf, pred_i[j_i].clone())

    # pts3d[j] = pred_j[j_i].clone()
    done = {i, j}
    if has_im_poses:
    ...
    while todo:
        ...
        if i in done:
            ...
            # breakpoint()
            static_mask_j = ~(dymask_j[i_j].bool())
            s, R, T = rigid_points_registration(pred_j[i_j][static_mask_j], pts3d[j][static_mask_j], conf=conf_j[i_j][static_mask_j])
