...
class DeltaPointCloudOptimizer(BasePCOptimizer):
    ...
    def __init__(self, 
        ...
        if self.is_static:
            ...
            dynamic_masks = [torch.zeros(self.imshape[0], self.imshape[1], dtype=torch.bool, device=self.device) for _ in range(self.n_imgs)]
        else:
            if self.input_dynamic_masks is None or self.use_self_dynamic_mask:
                dynamic_masks = self.get_motion_mask_from_pairs(*args)
            else:
                dynamic_masks = self.input_dynamic_masks

        dynamic_masks = torch.stack(dynamic_masks).bool()
        self.register_buffer('dynamic_masks', dynamic_masks)
        ...
        # self.delta_poses = nn.Parameter(self.rand_pose((self.n_imgs, 1+self.POSE_DIM)))  # pairwise poses
        # self.delta_adaptors = nn.Parameter(torch.zeros((self.n_imgs, 2)))  # slight xy/z adaptation
    ...
    def get_motion_mask_from_pairs(self, view1, view2, pred1, pred2):
        ...
        dynamic_masks = [[] for _ in range(self.n_imgs)]
        ...
        for i in range(self.n_imgs):
            dynamic_masks[i] = torch.stack(dynamic_masks[i]).mean(dim=0) > self.motion_mask_thre
    ...
    def get_delta_adaptors(self):
        adapt = self.delta_adaptors
    ...
    def get_delta_norm_scale_factor(self):
        if self.norm_pw_scale:
            # normalize scales so that things cannot go south
            # we want that exp(scale) ~= self.base_scale
            return (np.log(self.base_scale) - self.delta_poses[:, -1].mean()).exp()
        else:
            return 1  # don't norm scale for known poses

    def get_delta_scale(self):
        scale = self.delta_poses[:, -1].exp()  # (n_edges,)
        scale = scale * self.get_delta_norm_scale_factor()
        return scale
    
    def get_delta_poses(self):
        RT = self._get_poses(self.delta_poses)
        scaled_RT = RT.clone()
        scaled_RT[:, :3] *= self.get_delta_scale().view(-1, 1, 1)  # scale the rotation AND translation
        return scaled_RT
    ...
    def forward(self, cur_iter, stage='1'):
        ...
        # delta_pose = self.get_delta_poses()
        # delta_adapt = self.get_delta_adaptors()

        # breakpoint()
        aligned_pred_i = geotrf(pose_ei, self._stacked_pred_i)
        aligned_pred_j = geotrf(pose_ei, self._stacked_pred_j)
        # geotrf(pose_ei, self._stacked_pred_j)
        ...
        dynamic_mask2 = self.dynamic_masks[self._ej]
        dynamic_mask2 = rearrange(dynamic_mask2, 'n h w -> n (h w)')
        # lj = self.dist(proj_pts3d[self._ej], aligned_pred_j, weight=self._weight_j).sum() / self.total_area_j
        lj = self.dist(proj_pts3d[self._ej], aligned_pred_j, weight=self._weight_j)
        lj = (lj * (~dynamic_mask2).float()).sum() / (~dynamic_mask2).sum()
    ...
    def get_flow2d_loss_monst3r(self, cur_iter):
        ...
        dynamic_masks_all = self.dynamic_masks.unsqueeze(1)
