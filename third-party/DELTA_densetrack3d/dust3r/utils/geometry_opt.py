...
class DepthWarpWithFlow(torch.nn.Module):
    def __init__(self):
        super(DepthWarpWithFlow, self).__init__()
        self.base_coord = None
    ...
    @torch.no_grad()
    def get_inbound_mask(self, base_coord, flow_1_2):
        target_range = base_coord + flow_1_2.permute([0, 2, 3, 1])
        oob_mask = (target_range[..., 0] < 0) | (target_range[..., 0] > self.W-1) | (
            target_range[..., 1] < 0) | (target_range[..., 1] > self.H-1)
        return ~oob_mask[:, None, ...]
    ...
    # @torch.no_grad()
    def get_warped_depth(self, base_coord, flow_1_2, depth_2):
        B, C, H, W = flow_1_2.shape
        sample_grids = base_coord + flow_1_2.permute([0, 2, 3, 1])
        sample_grids[..., 0] /= (W - 1) / 2
        sample_grids[..., 1] /= (H - 1) / 2
        ...
        warped_depth = F.grid_sample(
            depth_2, sample_grids, align_corners=True)
        return warped_depth

    def forward(self, flow_1_2, depth_2):
        B, _, H, W = flow_1_2.shape
        if self.base_coord is None:
            self.init_grid([H, W], device=flow_1_2.device)
        base_coord = self.base_coord.expand([B, -1, -1, -1])
        inbound_mask = self.get_inbound_mask(base_coord, flow_1_2)

        warped_depth = self.get_warped_depth(base_coord, flow_1_2, depth_2)
        return warped_depth, inbound_mask