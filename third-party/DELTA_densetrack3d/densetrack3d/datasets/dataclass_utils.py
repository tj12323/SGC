...
def _dataclass_list_from_dict_list(dlist, typeannot):
    ...
    if issubclass(cls, tuple) and hasattr(cls, "_fields"):  # namedtuple
        ...
        dlist_T = zip(*dlist)
        res_T = [_dataclass_list_from_dict_list(key_list, tp) for key_list, tp in zip(dlist_T, types)]
        ...
        all_vals_res = _dataclass_list_from_dict_list([k for obj in dlist for k in obj.values()], val_t)
    ...
    fieldtypes = {
        f.name: (_unwrap_type(f.type), _get_dataclass_field_default(f)) for f in dataclasses.fields(typeannot)
    }
